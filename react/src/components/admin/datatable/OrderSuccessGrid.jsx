...
export default function OrderSuccessGrid(props) {
  ...
  const paymentMethodBodyTemplate = (rowData) => {
    return (
      <Tag
        value={rowData.order_status.name}
        severity={getSeverity(rowData.order_status.name)}
      />
    );
  };
  const paymentMethodItemTemplate = (option) => {
    ...
    return (
      <Tag
        value={option === "Đã giao hàng" ? "Đã giao hàng" : ((option === "Hoàn thành" ? "Hoàn thành" : "Yêu cầu trả hàng" ))}

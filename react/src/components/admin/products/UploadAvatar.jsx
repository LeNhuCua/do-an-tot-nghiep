...
function UploadAvatar({ image, setImage , imageCurrent }) {
  console.log(image);
  const onDrop = useCallback(
    (acceptedFiles) => {
      // Thêm các ảnh được chọn vào mảng `image`
  ...
  const removeImage = (image) => {
    ...
    const index = newImage.indexOf(image);
    if (index !== -1) {
      ...
      <div>
        {/* {Object.values(image).length > 0 && (
          <div className="p-4 grid grid-cols-1 xl:grid-cols-2 gap-3 mt-4">
            {Object.values(image).map((image,index) => (

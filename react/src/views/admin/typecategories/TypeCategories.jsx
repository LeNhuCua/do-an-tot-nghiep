...
const TypeCategories = () => {
  ...
  const { typeCategories, products } = state;
  ...
  const ApiExcel = `${API}/api/typeCategories/importExcel`;
  ...
  const showDetail = async (id) => {
    const getDetail = await typeCategories.find(
      (typeCategories) => typeCategories.typeCategoryId === id
    );
  ...
  const actionButtons = (rowData) => {
    return (
      <>
        <Tippy content="Sửa">
          <Link to={`/quantri/danhmuccon/chinhsua/${rowData.typeCategoryId}`}>
        ...
        <Tippy content="Chi tiết">
          <IconButton
            ...
            onClick={() => showDetail(rowData.typeCategoryId)}
          >
            <BiShow />
          </IconButton>
        </Tippy>
    ...
    return (
      <div>
        <Tippy content="Thêm mới">
          <Link className="block" to={"/quantri/danhmuccon/taomoi"}>
  ...
  function getIds(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      let obj = arr[i];
      ids.push(obj.typeCategoryId);
    }
    return ids;
  }

  const deleteSelectedData = async () => {
    let _data = typeCategories.filter((val) => selectedData.includes(val));
    ...
    await axiosClient
      ...
      .then((response) => {
        if (response.data.status === 200) {
          dispatch({
            type: "SET_TYPECATEGORIES",
            payload: typeCategories.filter(
              (product) => !getIds(selectedData).includes(product.typeCategoryId)
            ),
          });
          dispatch({
            type: "SET_PRODUCTS",
